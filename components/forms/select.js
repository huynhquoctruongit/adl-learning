...
export const Select = ({
  ...
  return (
    <div className="relative mb-3 text-sm max-w-[400px] min-w-[160px]">
      <input
        disabled={disabled}
        ...
        className={`w-full duration-200 text-caption-1 py-[10px] pr-[40px] px-[12px] flex items-center last:mr-0 rounded-[8px] content-center focus:outline-none ${cls} ${className}`}
        {...rest}
      ></input>
      <img
      ...
      />
      {focus ? (
        <div className={`absolute z-10 bg-white border-[1px] border-purple rounded-[8px] mt-3 w-full overflow-hidden`}>
          {options.map((item,index) => (
            <p
              key={index}
              className="hover:bg-purple-light p-3 cursor-pointer"
              onClick={() => {
                getData && getData(item), getData && setValueInput(item.value);
              }}
            >
              {item.value}
            </p>
